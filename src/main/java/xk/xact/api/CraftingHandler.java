...
/**
 * This handles the crafting of the recipes for the Crafter machine and for the
 * Chips.
 */
public abstract class CraftingHandler {
	 ...
	 *
	 * @param device
	 *            the ICraftingDevice from which to reference the
	 *            AvailableInventories
	 */
	public static CraftingHandler createCraftingHandler(
			final ICraftingDevice device) {
	...
	/**
	 * Whether if the specified recipe can be crafted. In other words, if there
	 * are enough items to craft it.
	 *
	 * @param recipe
	 *            the recipe to check.
	...
	/**
	 * Manages the crafting of a recipe. Fires the crafting event with the
	 * ingredients provided.
	 *
	 * @param recipe
	 *            the CraftRecipe from which to take ingredients of the recipe.
	 * @param craftMatrix
	...
	public ItemStack getRecipeResult(CraftRecipe recipe, InventoryCrafting craftMatrix) {
		...
		if (craftMatrix != null) {
			ItemStack stack = recipe.getRecipePointer().getOutputFrom(
					craftMatrix);
			if (stack == null)
				stack = CraftingManager.getInstance().findMatchingRecipe(
						craftMatrix, device.getWorld());
			if (stack != null)
				craftedItem = stack;
		}
	 ...
	 * iterate through them until it's correctly placed.
	 * <p/>
	 ...
	 *
	 * @param stack
	 *            the stack to be added to the inventories.
	 * @return true on success. False if the stack couldn't be completely merged
	 *         to the inventories.
	...
	/**
	 * Gets the amount of items of the same kind of the passed stack on the
	 * available inventories.
	 *
	 * @param recipe
	 *            the CraftRecipe to check
	 * @param stack
	 *            the stack to compare with.
	 * @param countAll
	 *            whether if this should keep counting after finding enough.
	 *            Enough implies that the amount found is equal or higher than
	 *            the amount required.
	...
	/**
	 * Will generate a temporary crafting grid based on the recipe's
	 * ingredients. The items that populate the grid will be taken from the
	 * available inventories.
	 *
	 * @param recipe
...
//			System.out.println(contents[i]);
	...
	/**
	 * Gets the amount of missing ingredients. The array contains the amount
	 * (associated by index with the ingredient it represents) missing of that
	 * item.
	 *
	 * @param recipe
	 ...
	 * @param recipe
	 *            the CraftRecipe representation of the recipe.
	 * @return an array of ItemStack, each of which has as stackSize the amount
	 *         missing.
	...
	/**
	 * A string listing the missing ingredients for the specified recipe. Useful
	 * to tell the user what's missing.
	 */
