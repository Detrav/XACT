...
public class ContainerRecipe extends Container implements
	...
	private void buildContainer() {
		...
		for (int i = 0; i < 3; i++) {
			for (int e = 0; e < 3; e++) {
				this.addSlotToContainer(new Slot(internalInventory, i * 3 + e,
						e * 18 + 44, i * 18 + 24) {
					@Override
					public boolean canTakeStack(EntityPlayer player) {
						return false;
	...
	@Override
	public ItemStack slotClick(int slotID, int buttomPressed, int flag,
			EntityPlayer player) {
		...
		if (1 == slotID && slotID < 10) {
			...
			if (flag == 2) { // interact with the hot-bar
				ItemStack invStack = player.inventory
						.getStackInSlot(buttomPressed);
				if (invStack != null) {
	...
	private void updateOutputSlot() {
		ItemStack[] gridContents = Arrays.copyOf(
				internalInventory.getContents(), 9);
		CraftRecipe recipe = RecipeUtils.getRecipe(gridContents,
				player.worldObj);

		Slot outputSlot = (Slot) inventorySlots.get(0);
		ItemStack item = null;
		if (recipe != null) {
			InventoryCrafting grid = InventoryUtils
					.simulateCraftingInventory(gridContents);
			item = recipe.getRecipePointer().getOutputFrom(grid);
		}
		outputSlot.putStack(item);

		int notify = item == null ? 1 : 0;
		// todo: notify the GuiRecipe that the recipe has changed. 0 means has
		// recipe, 1 means no recipe. (packet 0x05)
	}
