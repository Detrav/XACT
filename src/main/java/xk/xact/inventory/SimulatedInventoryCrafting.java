...
class SimulatedInventoryCrafting extends InventoryCrafting {
	...
	/**
	 * Fills the crafting grid with the specified ingredients. Note: fill order
	 * is right then down.
	 *
	 * @param ingredients
	 *            a ItemStack[9] containing all the ingredients.
	 */
	public void setContents(ItemStack[] ingredients) {
		for (int slot = 0; slot < 9; slot++) {
			inventory.setInventorySlotContents(slot,
					ingredients.length > slot ? ingredients[slot] : null);
		}
		updateContainer();
	}
