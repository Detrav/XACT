...
public class CraftingProject {
	...
	/**
	 * Gets an ordered list of all the raw materials involved in this Crafting
	 * Project.
	 */
	...
	private double count(ItemStack item, CraftRecipe recipe, double multiplier) {
		...
		for (ItemStack ingredient : recipe.getCompressedIngredients()) {
			...
			if (InventoryUtils.similarStacks(ingredient, item, false)) {
				...
				double nextMultiplier = multiplier * amount
						/ recipe2.getResult().stackSize;
	...
	public static CraftingProject readFromNBT(NBTTagCompound compound) {
		...
		NBTTagCompound nbt = (NBTTagCompound) compound
				.getTag("craftingProject");
		if (nbt == null)
			return project;

		// Read target item
		ItemStack item = Utils.readStackFromNBT((NBTTagCompound) nbt
				.getTag("targetItem"));
