...
public class CrateInventory implements IInventoryAdapter {
	...
	public Iterator<ItemStack> iterator() {
		try {
			...
			if (e.getLocalizedMessage().equals("Can't be called client-side.") && ConfigurationManager.DEBUG_MODE)
				Utils.logException("CrateInventory.class: " + e.getLocalizedMessage(), e, false);
			else if (!e.getLocalizedMessage().equals("Can't be called client-side.") && !ConfigurationManager.DEBUG_MODE)
				Utils.logException("Exception in CrateInventory.class: " + e.getLocalizedMessage(), e, false);
		}
