...
public class ChipHandler extends TemplateRecipeHandler {
	...
	class EncoderRecipe extends CachedRecipe {
		...
		public EncoderRecipe(int width, int height, Object[] items,
				ItemStack out, ItemStack chip) {
	...
	public void loadUsageRecipes(ItemStack ingredient) {
		if (CraftManager.isEncoded(ingredient)) {
			...
			CraftRecipe recipe = RecipeUtils.getRecipe(ingredient,
					FMLClientHandler.instance().getClient().theWorld);
			if (recipe != null) {
				EncoderRecipe recipeT = new EncoderRecipe(3, 3,
						recipe.getIngredients(), recipe.getResult(), ingredient);
